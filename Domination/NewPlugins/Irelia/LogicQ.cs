...
namespace DominationAIO.NewPlugins
{
    ...
    {
        public static void QW(AIBaseClient target)
        {
            if (target == null || !target.IsValid || !target.IsValidTarget(float.MaxValue))
                return;

            if (Irelia.W.IsReady())
            {
                if (Irelia.W.IsCharging)
                {
                    var farmpred = Irelia.W.GetLineFarmLocation(GameObjects.EnemyMinions.Where(i => i != null && i.IsValidTarget(800) && ReturnUnderTower(i.Position)).ToList());
                    if (farmpred.MinionsHit >= 2)
                    {
                        Irelia.W.ShootChargedSpell(farmpred.Position);
                        return;
                    }
                }
                else
                {
                    if (Irelia.Q.IsReady())
                    {
                        var farmpred = Irelia.W.GetLineFarmLocation(GameObjects.EnemyMinions.Where(i => i != null && i.IsValidTarget(800) && ReturnUnderTower(i.Position) && i.Distance(target) <= GameObjects.Player.Distance(target) + 50).ToList());
                        if (farmpred.MinionsHit >= 2)
                        {
                            Irelia.W.StartCharging();
                            return;
                        }
                    }
                }
